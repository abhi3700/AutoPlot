...
#     py.io.write_html(fig, file= cp_plot_html_file)    # M-2
...
#     py.io.write_html(fig, file=er_plot_html_file)     # M-2
# ------------------------------------------------------------------------------------------------------------------------------------------------------------------
"""
...
#     py.io.write_html(fig, file=unif_plot_html_file)   # M-2
...
def init():
    ...
    # wb = xw.Book.caller()
    wb = xw.Book('ASFE1_QC_LOG_BOOK.xlsm')
...
def button_run():
    ...
    # copy a sheet and paste into another sheet and skiprows 8
    df_asfe1_cp = excel_file.parse(sht_name_cp, skiprows=skiprows_cp)
    # The final dataframe with required columns
    df_asfe1_cp = df_asfe1_cp[sht_cp_columns]
    # replacing the empty cells with 'NIL'
    df_asfe1_cp['Remarks'].fillna('.', inplace=True)
    # replacing the empty cells with 'NIL'
    df_asfe1_cp['Delta CP 0.5u'].fillna('NIL', inplace=True)
    # replacing the empty cells with 'NIL'
    df_asfe1_cp['Delta CP AC'].fillna('NIL', inplace=True)
    # dropping rows where at least one element is missing
    df_asfe1_cp = df_asfe1_cp.dropna()
    ...
    # copy a sheet and paste into another sheet and skiprows 8
    df_asfe1_er = excel_file.parse(sht_name_er, skiprows=skiprows_pr)
    # The final Dataframe with 5 columns for plot: x-1, y-4
    df_asfe1_er = df_asfe1_er[sht_er_columns]
    # replacing the empty cells with 'NIL'
    df_asfe1_er['Remarks'].fillna('.', inplace=True)
    # dropping rows where at least one element is missing
    df_asfe1_er = df_asfe1_er.dropna()
